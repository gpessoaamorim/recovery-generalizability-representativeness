...
*IMPORT deaths file
import delim using "K:\QNAP\RECOVERY-deidentified\Datasets\INT04_Civil_Registration\0079\DP_INT04_Deident_Civ_Reg_2022-10-20_12-12-09.csv", delim(",") clear
keep study reg_date_of_death
rename study study_id
rename reg deathdat
save "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\deaths.dta", replace

*IMPORT age and randomisation dates
import delim using "K:\QNAP\RECOVERY\Raph\Rand_Dates\0079\DP_Date_Of_Rand_With_Age_2022-11-02_13-31-55.csv", delim(",") clear
keep study age randomisation_date
rename study study_id
save "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\age_rand.dta", replace

*IMPORT HES file
import delim using "K:\QNAP\RECOVERY\Raph\HES_2022_09_19\DP_INT02_HES_Deidentified_2022-10-25_09-42-26.csv", delim(",") stringcols(37) clear
rename study_number study_id

*Merge HES with age, randomisation dates and deaths
merge m:1 study_id using "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\age_rand.dta"
drop if _m == 2
drop _m
rename randomisation_date rand_date
merge m:1 study_id using "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\deaths.dta"
drop if _m == 2
drop _m

*Clean up HES file
foreach var of varlist admidate disdate epistart epiend opdate_01-opdate_24 rand_date deathdat {
gen `var'copy = date(`var', "YMD")
}
drop admidate disdate epistart epiend opdate_01-opdate_24 rand_date deathdat
rename *copy *
foreach var of varlist admidate disdate epistart epiend opdate_01-opdate_24 rand_date deathdat {
format `var' %td
}
order diag_01-diag_20 opertn_01-opertn_24
foreach var of varlist diag_01-opertn_24 {
...
foreach var of varlist diag_01-diag_20 {
replace `var' = substr(`var',1,3) if regexm(`var', "-$") == 1
replace `var' = substr(`var',1,3) if regexm(`var', " $") == 1
replace `var' = substr(`var',1,3) if regexm(`var', "[A-z]$") == 1
}
...
save "K:\QNAP\RECOVERY\Raph\HES\recovery_HES_79.dta", replace
use "K:\QNAP\RECOVERY\Raph\HES\recovery_HES_79.dta", clear

*Identify spells
gen stillin = .
replace stillin = 0 if spelend == "Y"
replace stillin = 1 if spelend == "N"
sort study admidate epiorder stillin
order study admidate epiorder stillin epistart epiend
gen long idspell = 1 if epiorder == 1
replace idspell = sum(idspell)
egen long id = group(study)
...
bysort study (recorder): gen randepiorder = cond(randepi == 1, sum(randepi), 0)
...
bysort study: egen covidmax = max(covid)
gen covid_anydiag = 0
foreach var of varlist diag_01-diag_20 {
 replace covid_anydiag = 1 if `var' >= "U071" & `var' <= "U072"
}
bysort study: egen covid_anydiagmax = max(covid_anydiag)
bysort study (recorder): gen covidorder = cond(covid == 1, sum(covid), 0)
gen U071 = 0
foreach var of varlist diag_01-diag_20 {
...
bysort study: egen indexdate = max(indexdatetemp)
bysort idspell: egen indexdisdatetemp = max(disdatespell) if indexspell == 1
bysort study: egen indexdisdate = max(indexdisdatetemp)
...
bysort study: egen ethvalidmax = max(ethvalid)
gen ethgrp = .
...
bysort study_id: egen ethgrpmodemax = mode(ethgrp), maxmode
...
bysort study_id: egen `var'max = max(`var')
}
...
bysort study_id: egen rrt2max = max(rrt2)
gen rrt2date = admidate if rrt2 == 1 & admidate < indexdate & disdate > indexdate-1826
bysort study: egen rrt2datemin = min(rrt2date)
...
bysort study: egen `var'max = max(`var')
}
...
bysort study: egen heartmax = max(heart)
...
bysort study: egen obesitymax = max(obesity)
...
bysort study: egen diabetesmax = max(diabetes)
...
bysort study: egen psychmax = max(psych)
...
bysort study: egen alcoholmax = max(alcohol)
...
order study admidate epistart epiend disdate index* covid
sort study epistart

save "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\HES_79_READY_allrecs.dta", replace

use "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\HES_79_READY_allrecs.dta", clear

*28-day mortality
merge 1:1 study using "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\deaths.dta"
drop if _m == 2
drop _m
gen death = 0
replace death = 1 if deathdat != .
drop death28
gen deathdatnum = date(deathdat, "YMD")
drop deathdat
rename deathdatnum deathdat
gen death28 = 0
replace death28 = 1 if deathdat >= indexdate & deathdat <= indexdate+27

*Housekeeping

keep if indexdate <= 22614
keep if index != .
rename covidmax covid_primary
cap drop covid_anydiag
rename covid_anydiagmax covid_anydiag
rename U071max u071_anydiag
rename U072max u072_anydiag
rename U072only u072_only_anydiag
keep study admidate epistart epiend disdate indexdate covid_primary covid_anydiag u071_anydiag u072_anydiag u072_only_anydiag Metastasis-Mild_Liver MI disdatespell epikey extract_num procode3 sex rand_date deathdat age agegrp ethgrp charlson_score charlson_score_age charlgrp charlgrp_age rrtmax immunomax heartmax diabetesmax psychmax alcoholmax obesitymax obesityspell frailty_score death deathdat death28 
rename extract_num extract
rename charlgrp charlson_grp
rename charlgrp_age charlson_grp_age
rename *, lower
rename immuno immunosup
rename *max *
rename obesityspell obesity_index
rename psych psychiatric
drop disdate
rename disdatespell disdate
order study epikey rand_date index indexdate covid_primary covid_anydiag u071_anydiag u072_anydiag u072_only_anydiag admidate epistart epiend disdate procode3 age agegrp sex ethgrp metastasis-mild_liver mi charlson_score charlson_score_age charlson_grp charlson_grp_age immuno heart diabetes psych alcohol rrt obesity obesity_index frailty_score death deathdat death28 extract 
rename rrt rrt_algorithm
drop procode admidate epistart epiend disdate death deathdat
...
*MERGE with postcode file
merge 1:1 epikey using "K:\QNAP\RECOVERY\Raph\HES_2022_09_19\HES_Postcode\HES_79_homeadd.dta"
drop if _m == 2
drop _m

save "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\HES_79_READY.dta", replace

*Create output file for analysis
use "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\HES_79_READY.dta", clear
...
drop if age >105
gen agebroad = .
replace agebroad = 1 if age >= 15 & age <= 59
replace agebroad = 2 if age >= 60 & age <= 69
replace agebroad = 3 if age >= 70 & age <= 79
replace agebroad = 4 if age >= 80 & age <= 89
replace agebroad = 5 if age >= 90 & age <= 105
...
*tab deathint agebroad if death28 == 1, nofreq col
outsheet * using ///
"K:\QNAP\RECOVERY-deidentified\Generalizability and representativeness project\RECOVERY_generalizability_representativeness\Intermediate outputs\Datasets\Recovery_HES_79_v1.csv", replace delim(",")
...
outsheet * using ///
"K:\QNAP\RECOVERY-deidentified\Generalizability and representativeness project\RECOVERY_generalizability_representativeness\Intermediate outputs\Datasets\Recovery_HES_79_deathinterval_v1.csv", replace delim(",")

*Create output file for local authority counts
use "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\HES_79_READY.dta", clear
merge m:1 lsoa using "K:\uhce\Libraries\Geographical\soal_imd19_lookup.dta"
drop if _m == 2
drop _m
keep lacode
gen total = 1
collapse (sum) total, by(lacode) 
rename lacode ladcd
rename total freq_recovery
save "K:\QNAP\RECOVERY\Raph\Generalisability and representativeness\recovery_LA_freqcounts_79_v1.dta", replace
outsheet * using ///
"K:\QNAP\RECOVERY-deidentified\Generalizability and representativeness project\RECOVERY_generalizability_representativeness\Intermediate outputs\Datasets\recovery_LA_freqcounts_79_v1.csv", replace delim(",")
